...
df %>%
  ...
  column_spec(2,
              background = "#FFAABB") %>% kable_styling()
```
...
![](figures/likert-scale-bad-red.svg)
...
df %>%
  knitr::kable(format = 'html') %>%
  column_spec(2,
              background = "#99DDFF") %>% kable_styling()
```
...
```{r echo = TRUE}
df <- df %>% 
  mutate(`Problem Expectation` = 
           11 - `Problem Expectation`)
```


```{r echo = TRUE, eval = FALSE}
# Alternatively manually assign values
df <- df %>% mutate(`Problem Expectation` =
  recode(`Problem Expectation`,
    `1` = 10,
    `2` = 9,
    `3` = 8,
    `4` = 7,
...
df_mr %>%
  ...
  row_spec(c(1,4,6), background = "#99DDFF") %>% kable_styling()
```
...
df %>%
  ...
  row_spec(c(2:3,5), background = "#99DDFF") %>% kable_styling()
```
...
```{r}
df <- df %>%
  rename_with(~ paste0("CUEX", 1:3), # function to generate name
              .cols = c(1:3)) # where to apply?
```
...
```{r, echo = FALSE}
df %>%
  knitr::kable(format = 'html') %>% kable_styling()
```
